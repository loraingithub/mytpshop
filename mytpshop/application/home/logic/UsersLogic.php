...
//			}
    ...
    /*邀请码*/
    /*public function yqmyz($topyqm)
    {
        //$is_validated = 0 ;
        $user = M('users')->where("yqm", $topyqm)->find();
        //var_dump($user);
        if ($user != ''){
            return array('status' => 1, 'msg' => '邀请码填写正确');
        }
        else{
            return array('status' => -1, 'msg' => '此邀请码不存在，请重新填写');
        }
    }*/
    ...
    public function reg($username,$password,$password2, $push_id=0){
        ...
        $yqm=$yqm0.$yqm1;
        $map['yqm'] = $yqm;  //自身邀请码
        $sjyqm = trim(session('topyqm'));
        //$map['topyqm'] = session('topyqm');  //上级邀请码
        $map['topyqm'] = $sjyqm;
        $map['httpurl'] = session('httpurl');  //url地址

        //first_leader, second_leader, user_id   三级ID
        $topyqm = session('topyqm');
        //$user = M('users')->where("yqm",$topyqm)->find();
        $user = M('users')->where("yqm",$sjyqm)->field(array('first_leader','second_leader','user_id'))->find();
        //$user->field(array('first_leader','second_leader','user_id'))->find();
        $map['first_leader'] = $user['user_id'];
        $map['second_leader'] = $user['first_leader'];
        $map['third_leader'] = $user['second_leader'];

        //$user = M('users')->where("yqm",$yqm)->select();

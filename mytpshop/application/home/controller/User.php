...
//    		$cartLogic->login_cart_handle($this->session_id,$res['result']['user_id']);  //用户登录后 需要对购物车 一些操作
    ...
    public function reg(){
        //echo $_SERVER['HTTP_HOST'];  // 获取当前页面URL
    	if($this->user_id > 0){
            $this->redirect('Home/User/index');
        }
...
//            $this->verifyHandle('user_reg');
            ...
            $topyqm = I('get.topyqm','');
            $_SESSION['httpurl'] = $_SERVER['HTTP_HOST'];   // 获取当前页面URL
            $_SESSION['topyqm'] = I('post.topyqm');
...
//            $cartLogic->login_cart_handle($this->session_id,$data['result']['user_id']);  //用户登录后 需要对购物车 一些操作
    ...
    */
    ...
    public function order_confirm(){
        $orderid     =      $_GET['id'] ;
        $goods_id    =      M('order_goods')->where(" order_id='$orderid' ")->getField('goods_id') ;
        $info        =      M('order_goods')->where(" order_id='$orderid' ")->select() ;
        ...
        if($data['status'] != 1){
            ...
            if($goods_id==147){
                ...
                $info = M('users')->where(" user_id= '$id' ")->setInc('is_daili',1);
                $daili = new fx() ;
                $fenxiao = $daili->daili($id) ;
            }else{
            //收货后奖金派发
            $a           =   new fx() ;   
            for ($i=0; $i <$num ; $i++) { 
                ...
                $gnum    =   $info[$i]['goods_num'] ;
                $b       =   $a->shopfx($uid,$gid,$gnum) ;
                $c       =   $a->shopdl($uid,$gid,$gnum) ;
                  }
            }
                //每笔订单盈利及分佣金额存入订单表
                $info      =   M('order')->where(" order_id='$orderid' ")->find() ;
                $fydlmoney =   $_SESSION['dlmoney'] + $_SESSION['fxmoney'] ;
                $lrmoney   =   $info['total_amount'] - $fydlmoney-$info['shipping_price'] ;
                $fydl      =   M('order')->where(" order_id='$orderid' ")->setField('fydlmoney',$fydlmoney) ;
                $lrmoney   =   M('order')->where(" order_id='$orderid' ")->setField('lrmoney',$lrmoney) ;
                //订单完成后置零
                $_SESSION['dlmoney']  =  0 ;
                $_SESSION['fxmoney']  =  0 ;
                ...
                if($goods_id!=147){
                //将成功购买商品的ID拼装存入用户表
                $info1   =   M('users')->where("user_id='$uid' ")->getField('fx_shop') ;
                $array   =   explode(',',$info1) ;//此处分割字符为数组，最后多出一个空元素
                array_pop($array) ;
                $hebin   =   array_merge($fxsz,$array) ;
                $quchong =   array_unique($hebin);
                $fenge   =   implode(",",$quchong);
                $fenge   =   $fenge."," ;
                $end     =   M('users')->where("user_id='$uid' ")->setField('fx_shop',$fenge) ;
                }
    ...
    public function fendian()
    {
       ...
       // $user_info = $user_info['result'];
        //$this->assign('user',$user_info);
        //$mes = M('users')->where('user_id','2658')->find();


        $session = session('');   //获取session里面所有存在的值
        $userid = $session['user']['user_id'];  //取session里面的user_id
        $mes = M('user_store')->where('user_id',$userid)->find();
        $time = $mes['store_time'];
...
//        var_dump($mes) ;exit;
        if($mes != ''){
            $this->assign('mes',$mes);
            $this->assign('strtime',$strtime);  //分配时间
            return $this->fetch();
        }else{
            //$this->success('您还没有自己的分店', 'User/fendian');
            echo"<script>alert('您还没有属于自己的分店');history.go(-1);</script>";
        }
...
/**/
    ...
    public function fenxiao(){
        $logic = new UsersLogic(); //  获取用户登录信息
        $user = $logic->get_info($this->user_id);   
        $user = $user['result'];
        $id = $user['user_id']; //获取用户ID
        $supplier_count=M('rebate_log')->where('user_id',$id)->count();     //获取总数
        $page = new Page($supplier_count, 5);  //分页数
        $show = $page->show();      
        $data = M('rebate_log')->where('user_id',$id)->limit($page->firstRow, $page->listRows)->select();       //查询分销列表,并进行分页查询
